...
for epoch in range(num_epochs):
    ...
    # Log epoch loss and validation ELBO to W&B (after computing val_elbo)
    wandb.log({
...
# -------------------------------
# Final sampling and upload to W&B (after training, generated by AI)
# -------------------------------

with torch.no_grad():
    n_samples = 16
    n_steps = 50
    samples = vdm_ema_final.sample(batch_size=n_samples, n_sample_steps=n_steps, clip_samples=True)
    samples = samples.clamp(-1, 1)
    samples_vis = (samples + 1.0) / 2.0  # [-1,1] -> [0,1]
    grid = make_grid(samples_vis.cpu(), nrow=int(math.sqrt(n_samples)), padding=2)

wandb.log({
    "final/samples_grid": wandb.Image(grid, caption=f"Final EMA samples ({n_samples})")
})
